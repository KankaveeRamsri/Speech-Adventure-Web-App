...
export default function SayAfterMeActivity({ item, onComplete }: Props) {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      setEvalResult({
        score: result.score,
        status: result.status as PhonicsAttemptStatus,
        feedback: result.feedback,
        isMock: result.isMock,
      });
      setListened(true);
    } catch {
      // Fallback: let child proceed anyway
    ...
    <div className="space-y-6">
      ...
      {!evalResult && (
        <div className="flex flex-col items-center gap-3">
          <p className="text-xs text-text-muted/70 font-medium uppercase tracking-wider">ลองพูดตาม</p>
          <button
            type="button"
            onClick={isRecording ? recorder.stopRecording : recorder.startRecording}
            disabled={recorder.state === "requesting_permission" || recorder.state === "processing" || hasRecording}
            ...
            className={`w-16 h-16 rounded-full flex items-center justify-center transition-all active:scale-[0.94] disabled:cursor-not-allowed ${
              isRecording ? "bg-red-100 dark:bg-red-950/40 text-red-500 animate-pulse"
                : hasRecording ? "bg-border/30 text-text-muted/40"
          ...
          <span className="text-sm text-text-muted">
            {isRecording ? "กำลังบันทึก... กดหยุดเมื่อพูดเสร็จ"
              : hasRecording ? "บันทึกเสร็จแล้ว ✅"
              : "กดเพื่อบันทึกเสียง"}
          </span>
        </div>
      )}

      {/* After recording: play back + evaluate */}
      ...
      {evalResult && (
        <div className={`animate-slide-up rounded-xl px-4 py-3 text-center border ${
          evalResult.status === "passed" ? "bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800/50"
            : evalResult.status === "almost" ? "bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-700/50"
            : "bg-surface border-border"
        }`}>
          <p className={`font-bold text-base ${evalResult.status === "passed" ? "text-green-700 dark:text-green-400" : evalResult.status === "almost" ? "text-amber-700 dark:text-amber-400" : "text-text"}`}>
            {evalResult.feedback}
          </p>
